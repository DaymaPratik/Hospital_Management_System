...
const SalaryReport = () => {
 ...
 const totalSalary = staffList.reduce((acc, staff) => acc + Number(staff.salary || 0), 0);

  return (
    <div className={styles.salaryContainer}>
      <h2>💰 Monthly Salary Report</h2>
      <p>Total salary expenditure and individual staff salaries.</p>

      <ul className={styles.salaryList}>
        {staffList.map((staff, index) => (
          <li key={index} className={styles.salaryItem}>
      ...
      <div className={styles.total}>
        <strong>Total Expenditure:</strong> ₹{totalSalary}
      </div>
    </div>
  );
};
